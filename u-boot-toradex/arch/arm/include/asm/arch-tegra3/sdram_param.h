...
/**
 * Defines the SDRAM parameter structure. Contains the config options
 * exposed by the Memory Controller and the External Memory Controller
 */
struct sdram_params {
	...
	u32 emc_dll_xform_dqs[8];
	u32 emc_dll_xform_quse[8];
	u32 emc_dli_trim_txdqs[8];
	u32 emc_dll_xform_dq[4];
	u32 wb_wait;
